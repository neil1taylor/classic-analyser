...
function getSteps(
  ...
  const phases = ['Authenticating', 'Shallow Scan', 'Deep Scan', 'Relationships', 'Complete'];

  const phaseToIndex = (p: string): number => {
    if (p.startsWith('Shallow Scan')) return 1;
    if (p.startsWith('Deep Scan') || p === 'VCFaaS Clusters' || p === 'TGW Route Reports') return 2;
    if (p.startsWith('Relationship')) return 3;
    return 0;
  };
  ...
  return phases.map((label, i) => {
    ...
    // If complete but has errors, last step still shows complete (errors shown separately)
...
const stepTooltips: Record<string, string> = {
  'Shallow Scan': 'Quick enumeration of all resource types and counts',
  'Deep Scan': 'Detailed data collection for each discovered resource',
  'Relationships': 'Mapping parent-child links between resources',
};
...
const ProgressIndicator: React.FC<ProgressIndicatorProps> = ({ onCancel }) => {
  const { collectionStatus, progress, errors, collectionDuration } = useData();
  ...
  if (collectionStatus === 'complete') {
    const durationText = collectionDuration
      ? `${(collectionDuration / 1000).toFixed(1)}s`
      : '';
    return (
      <InlineNotification
        kind="success"
        title="Collection complete"
        subtitle={`All resources collected successfully${durationText ? ` in ${durationText}` : ''}.${errors.length > 0 ? ` ${errors.length} warning(s).` : ''}`}
        lowContrast
  ...
  if (collectionStatus === 'error') {
    return (
      <InlineNotification
        ...
        subtitle={errors.length > 0 ? errors[errors.length - 1].message : 'An error occurred during data collection.'}
        lowContrast
        hideCloseButton={false}
      />
    );
  ...
  if (collectionStatus === 'cancelled') {
    return (
      <InlineNotification
        ...
        subtitle="Data collection was cancelled. Partial data may be available."
        lowContrast
        hideCloseButton={false}
      />
    );
  ...
  // Collecting state
  const steps = getSteps(progress.phase, collectionStatus, errors.length > 0);

  const percentage = progress.totalResources > 0
    ? Math.round((progress.completedResources / progress.totalResources) * 100)
    : 0;

  const statusText = progress.resource
    ? `${progress.resource}: ${progress.status || 'collecting'}`
    : progress.completedResources > 0
      ? `${progress.completedResources} of ${progress.totalResources} resources collected`
      : 'Initializing...';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {steps.map((step, i) => (
          <React.Fragment key={step.id}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 80 }}>
              ...
              {stepTooltips[step.label] ? (
                <Tooltip label={stepTooltips[step.label]} align="bottom">
                  <span style={{
                    fontSize: '0.75rem',
                    marginTop: '0.25rem',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <p style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600 }}>
            {progress.phase || 'Starting collection...'}
          </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <Button
            ...
            iconDescription="Cancel collection"
      ...
      <ProgressBar
        ...
        helperText={progress.phase ? `${progress.phase} — ${progress.completedResources} / ${progress.totalResources}` : 'Starting...'}
      />
    </div>
  );
};
