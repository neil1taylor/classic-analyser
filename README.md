...
- **VPC migration assessment** — 69 automated pre-requisite checks (34 compute, 20 network, 12 storage, 3 security), per-workload migration approach recommendation (Lift & Shift / Rebuild / Re-platform / Re-architect), cost comparison with IBM internal discount rates and regional price uplift, IKS/ROKS cluster analysis, wave planning, Assessment Template export, Terraform export, and DOCX reports. Profile matching uses curated IBM Classic-to-VPC mapping data (266 VSI + 42 BM profiles, 666 explicit bare metal mappings) with algorithmic fallback
